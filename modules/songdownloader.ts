...
export const downloadSong = (url: string): Promise<{status: boolean, res: string, id?: string, new: boolean}> => {
    ...
    return new Promise(resolve => {resolve({status: false, res: 'Spotify und YouTube Links werden unterstützt', new: false})});
}
...
export const downloadYoutubeSong = (url: string): Promise<{status: boolean, res: string, id?: string, new: boolean}> => {
    return new Promise(async (resolve) => {
        ...
        if(songsBlock.data.find(q => q.toLowerCase() == url.toLowerCase())) return resolve({status: false,res: 'Dieser Link ist von der Projektverwaltung gesperrt', new: false});

        if(blockId.has(url)) return resolve({status: false,res: 'Dieser Song wird bereits von einem anderen Spieler hochgeladen, bitte warte', new: false})

        let resFilename = `${url}.mp3`

        const data = await getYoutubeVideoData(url);
        if(!data) return resolve({status: false,res: 'Die Daten konnten nicht vom Youtube-Server abgerufen werden. Prüfe, ob der Link korrekt ist', new: false});
        if(data.views < YOUTUBE_LIMIT_ACCESS.viewsCount) return resolve({status: false, res: `Minimum erforderlich ${YOUTUBE_LIMIT_ACCESS.viewsCount} Ansichten`, new: false});
        if(data.duration < YOUTUBE_LIMIT_ACCESS.durationMinSeconds) return resolve({status: false, res: `Das Video muss mindestens ${system.secondsToString(YOUTUBE_LIMIT_ACCESS.durationMinSeconds)}`, new: false});
        if(data.duration > YOUTUBE_LIMIT_ACCESS.durationMaxSeconds) return resolve({status: false, res: `Das Video sollte maximal ${system.secondsToString(YOUTUBE_LIMIT_ACCESS.durationMaxSeconds)}`, new: false});
        if(data.likes < YOUTUBE_LIMIT_ACCESS.likesCount) return resolve({status: false, res: `Minimum erforderlich ${YOUTUBE_LIMIT_ACCESS.likesCount} mag`, new: false});
        if((((data.likes + data.dislikes) / 100) * data.dislikes) < YOUTUBE_LIMIT_ACCESS.dislikePercent) return resolve({status: false, res: `Nicht mehr als ${YOUTUBE_LIMIT_ACCESS.dislikePercent}% mag nicht`, new: false});
...
//Download video and save as MP3 file
        ...
        YD.on("finished", function(err, dataz) {
            blockId.delete(url)
            if(err){
                system.debug.error(`Fehler beim Herunterladen von Musik ${err}`)
                return resolve({status: false, res: `Beim Laden ist ein Fehler aufgetreten`, new: false});
            }
        ...
        YD.on("error", function(error) {
            system.debug.error(`Fehler beim Herunterladen von Musik ${error}`)
            return resolve({status: false, res: `Beim Laden ist ein Fehler aufgetreten`, new: false});
        });
...
export const downloadSpotifySong = (url: string): Promise<{status: boolean, res: string, id?: string, new: boolean}> => {
    return new Promise((resolve) => {
        ...
        if(!url.includes('spotify.com')){
            let data = url.split(':');
            if(data.length !== 3) return resolve({status: false, res: 'Der Spotify-Link ist nicht korrekt aufgeführt', new: false});
            if(data[0] !== 'spotify') return resolve({status: false, res: 'Der Spotify-Link ist nicht korrekt aufgeführt', new: false});
            if(data[1] !== 'track') return resolve({status: false, res: 'Der Spotify-Link ist nicht korrekt aufgeführt', new: false});
            url = `https://open.spotify.com/track/${url.split(':')[2]}`;
        } else {
            if(url.indexOf('https://open.spotify.com/track/') !== 0) return resolve({status: false, res: 'Der Spotify-Link ist nicht korrekt aufgeführt', new: false});
        }
        url = url.split('?')[0];
        let id = url.split(`/track/`)[1];
        if(blockId.has(id)) return resolve({status: false,res: 'Dieser Song wird bereits von einem anderen Spieler hochgeladen, bitte warte', new: false})
        if(songsBlock.data.find(q => q.toLowerCase() == id.toLowerCase())) return resolve({status: false,res: 'Dieser Link ist von der Projektverwaltung gesperrt', new: false});
        ...
        exec(`spotifydl ${url} -o ${targetDir}`, async (err, out, res) => {
            ...
            if(!r) return resolve({status: false,res: 'Falsche Daten wurden beim Laden empfangen', new: false});
            let name = r.split(' Song: ')[1].replace(/\\/g, '');
            if(!name) return resolve({status: false,res: 'Abrufen von Informationen beim Booten fehlgeschlagen', new: false});
            ...
            if(!resRenFilename) return resolve({status: false, res: 'Ein Fehler ist aufgetreten, versuche es erneut', new: false})
            ...
            //fs.rename(`${targetDir}/${resRenFilename}`, `${SONG_FOLDER}${newFilename}`, async function(err) {
                ...
                if(err) {
                    ...
                    return resolve({status: false, res: 'Ein Fehler ist aufgetreten, versuche es erneut', new: false})
                }
