...
CustomEvent.registerClient('boombox:removeIgnore', (player, id) => {
    ...
    if(indexB > -1){
        ...
        songsBlock.save()
        player.notify('Das Lied wurde aus der Sperrung entfernt', 'success')
    } else {
        player.notify('Song nicht in der Blockliste gefunden', 'error')
    }
})
...
CustomEvent.registerCef('boombox:take', ((player, id: string) => {
    ...
    if(!item || !item.isExists())
        return player.notify('Der Spieler wurde bereits genommen', 'error');
...
CustomEvent.registerCef('boombox:load', (player, id: string, type: MUSIC_GUI_SONG_TYPE, url: string) => {
    return new Promise(resolve => {
        ...
        if (!item.isPlayerHasAccess(player)) {
            player.notify('Du hast keinen Zugriff auf den privaten Spieler', 'error')
        ...
        if(type === 'player'){
            ...
            if(playerBlockDWN.has(player.dbid) && mp.config.announce){
                player.notify(`Warte ein paar Minuten, bevor du einen neuen Song herunterlädst`, 'error');
                return resolve(false)
            }
            if(dwnOneTime >= MUSIC_NEW_DOWNLOAD_ONE_TIME){
                player.notify(`Es gibt zu viele Spieler, die gerade neue Lieder hinzufügen, warte einfach ein paar Minuten`, 'error');
                return resolve(false)
            }
...
CustomEvent.registerCef('boombox:task', (player, id: string, task: MUSIC_GUI_TASKS, ...args: any[]) => {
    ...
    if (!musicPlayer.isPlayerHasAccess(player))
        return player.notify('Du hast keinen Zugriff auf den privaten Spieler', 'error');
...
export class BoxPlayer extends MusicPlayer {
    ...
    constructor(player: PlayerMp, item: ItemEntity) {
        ...
        this.handleColshape = colshapes.new(this.position, `Musikzentrum`, player => {
    ...
    tick() {
        ...
        if (!this.playList.length)
            return this.text = 'Wiedergabeliste ist leer';

        if (!this.currentSongUrl)
            return this.text = 'Warten auf ein Lied';

        if (![...this.players].length)
            return this.text = '';

        this.text = `Es läuft gerade:\n${this.currentSongName}`
    }
    ...
    takePlayer(player: PlayerMp): void {
        ...
        if(!user.isAdminNow() && this.ownerId !== user.id)
            return player.notify('Du kannst den Spieler nicht aufheben', 'error');
